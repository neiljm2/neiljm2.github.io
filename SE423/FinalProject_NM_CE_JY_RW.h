...
#define LADAR_LENGTH 228
xy ladar_pts[LADAR_LENGTH]; //xy data
...
float MaxAreaThreshold1 = 0.0;
float MaxColThreshold1 = 0.0;
float MaxRowThreshold1 = 0.0;
...
int16_t ball_idx1_x = 0;
int16_t ball_idx1_y = 0;
int16_t goal_idx1_x = 0;
int16_t goal_idx1_y = 0;
waypt waypoint;
...
void swi1_before_sm(void)
{
    ...
    if(calibration_state == 0){
        ...
        LeftVel = (0.1034)*(LeftWheel - LeftWheel_1)*1000;
        RightVel = (0.1034)*(RightWheel - RightWheel_1)*1000;

        if (newLinuxCommands == 1) {
            newLinuxCommands = 0;
            Kp_right_wall = LinuxCommands[0];
            Kp_front_wall = LinuxCommands[1];
            k_goal = LinuxCommands[2];
            k_ball = LinuxCommands[3];
            k_turn = LinuxCommands[4];
            k_obs  = LinuxCommands[5];
            //?? = LinuxCommands[6];
            //?? = LinuxCommands[7];
            //?? = LinuxCommands[8];
            //?? = LinuxCommands[9];
        ...
        if (NewCAMDataAprilTag1 == 1) {
            ...
            tagx = fromCAMvaluesAprilTag1[0];   // just call these variables to use the apriltag values  - yuan41
...
void swi1_after_sm(void){
    ...
    if((timecount%100) == 0) {
        DataToLabView.floatData[0] = (float)rb_pop();
        DataToLabView.floatData[1] = (float)rb_pop();
        DataToLabView.floatData[2] = (float)rb_pop();
        DataToLabView.floatData[3] = (float)rb_pop();
        DataToLabView.floatData[4] = (float)ball_idx1_y;
        DataToLabView.floatData[5] = (float)ball_idx1_x;
//        DataToLabView.floatData[4] = (float)waypoint.y;
//        DataToLabView.floatData[5] = (float)waypoint.x;
        DataToLabView.floatData[6] = (float)goal_idx1_y;
        DataToLabView.floatData[7] = (float)goal_idx1_x;
